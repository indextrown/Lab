...
class Mysql:
    ...
    # popup uuid 조회
    @staticmethod
    def get_popup_uuid(cursor, popup_id: int) -> str:
    ...
    # popup_recommend 삽입 메서드
    @staticmethod
    def insert_recommends(cursor, popup_id: int, recommend_ids: List[int]):
        ...
        for rid in recommend_ids:
            cursor.execute(sql, (popup_id, rid))
    ...
    def play(local: bool = True):
        ...
        for item in items:
            ...
            try:
                ...
                # uuid 조회
                popup_uuid = Mysql.get_popup_uuid(cursor, popup_id)

                # popup_image INSERT
                Mysql.insert_images(cursor, popup_id, dto.imageList)

                # ⭐ recommend INSERT (새 메서드)
                Mysql.insert_recommends(cursor, popup_id, dto.recommendIdList)

                # 지금까지의 DB 변경 내용을 영구적으로 저장
                conn.commit()
