...
class InstagramPostDTO:
    ...
    timestamp: str
    media_urls: List[str]
    post_type: str  # ⭐ 두쫀쿠 / 탕후루 / 마카롱 등 (GPT category_map과 연결)
...
class InstagramAPI:
    ...
    def get_recent_media(self, hashtag_id: str, post_type: str, limit: int = 5) -> List[InstagramPostDTO]:
    ...
    def play(config: dict):
        ...
        # 1️⃣ main 검색
        main_keyword = config["main"]
        main_id = api.get_hashtag_id(main_keyword)
        if main_id:
            results += api.get_recent_media(
                hashtag_id=main_id,
                post_type="인스타",
                limit=3
            )

        # 2️⃣ sub 검색 (개수만큼 반복 → 여기서 2번 더 호출됨)
        for keyword, post_type in config.get("sub", {}).items():
            hashtag_id = api.get_hashtag_id(keyword)
            if not hashtag_id:
                continue
        ...
        api.save_json(results, "popup.json")
